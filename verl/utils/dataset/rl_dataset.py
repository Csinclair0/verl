...
class RLHFDataset(Dataset):
        ...
        if self.filter_overlong_prompts:
            ...
            if processor is not None:
                ...
                def doc2len(doc) -> int:
                    try:
                        chat = doc[prompt_key]
                        # Only apply chat template if chat is a list of messages, otherwise use it directly as a string
                        if isinstance(chat, list):
                            ...
                            # chat is already a formatted string, use it directly
                            raw_prompt = chat
                ...
                def doc2len(doc) -> int:
                    try:
                        ...
                        if isinstance(chat, list):
                            ...
                            return len(
                                tokenizer.apply_chat_template(chat, add_generation_prompt=True, **apply_kwargs)
                            )
                        else:
                            # chat is already a formatted string, tokenize it directly
                            return len(tokenizer(chat, add_special_tokens=False))
                    except Exception:
                        print("Error processing one of the samples, skipping...")
                        traceback.print_exc()
                        return self.max_prompt_length + 1
    ...
    def __getitem__(self, item):
        ...
        if self.processor is not None:
            ...
            # Only apply chat template if chat is a list of messages, otherwise use it directly as a string
            if isinstance(chat, list):
                messages = self._build_messages({"prompt": chat, **row_dict})
                raw_prompt = self.processor.apply_chat_template(
