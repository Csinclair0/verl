...
from verl.models.mistral.megatron.modeling_mistral_packed import (
    convert_mistral_to_packed
)


class TestMistralPacked(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.original_model = MistralForCausalLM(self.config)
        self.packed_model = convert_mistral_to_packed(
            self.original_model, 
            self.config
        )
    ...
    def test_embedding_weight_copy(self):
        """Test that embedding weights are correctly copied."""
    ...
    def test_attention_weight_copy(self):
        """Test that attention weights are correctly copied."""
        # Check first layer attention weights
        original_q_proj = self.original_model.model.layers[0].self_attn.q_proj.weight
        packed_q_proj = (
            self.packed_model.model.layers[0].self_attn.attention.q_proj.weight
        )
    ...
    def test_forward_equivalence(self):
        """Test that the packed model produces equivalent outputs."""
        batch_size = 2
        seq_len = 10
        ...
        packed_logits_reshaped = packed_outputs['logits'].view(
            batch_size, 
            seq_len, 
            -1
        )
        ...
        # Outputs won't be exactly equal due to different implementations,
